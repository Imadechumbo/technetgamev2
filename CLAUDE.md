...
### PAR DE SCRIPTS DE DEPLOY — CANDIDATO A CONSOLIDAÇÃO (Fase C, ainda não decidida)
`scripts/deploy_backend_eb.py` (86 linhas, implementação real: polling de ambiente, validação de env vars, argparse) + `scripts/deploy_backend_aws_eb.py` (15 linhas, wrapper fino — só remapeia 3 nomes de env var: `AWS_EB_BUNDLE`→`BACKEND_BUNDLE_PATH`, `AWS_EB_S3_BUCKET`→`AWS_S3_DEPLOY_BUCKET`, `AWS_EB_VERSION_LABEL`→`BACKEND_VERSION_LABEL`, depois chama `deploy_backend_eb.main()`).

**Achado adicional (2026-07-04, durante investigação da Fase C):** os dois scripts são chamados por **workflows diferentes e não-equivalentes** — `deploy-production.yml` (mais antigo, não documentado em nenhum README numerado, último commit 2026-04-19) chama o wrapper; `release-auto-rollback.yml` (mais novo, é o pipeline V2.14 documentado em `README_V2_14_GITHUB_ACTIONS_AUTODEPLOY_ROLLBACK.txt`, último commit 2026-04-20, com `release_track`/concorrência/gates pós-deploy) chama o script real diretamente, sem precisar do wrapper. Ou seja: o nome de env var "nativo" do script real já é o usado pelo pipeline mais atual — o wrapper existe só pra manter o workflow mais antigo funcionando com nomes de secret diferentes.
